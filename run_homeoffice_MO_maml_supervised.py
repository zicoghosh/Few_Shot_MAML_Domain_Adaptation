...
from maml_da_src_supervised.mo_maml_da import MAML
...
def main():
    ...
    fileStart = 'officehome_mo_maml_checkpoint_order_' + str(args.order) + '_seed_' + str(args.seed) + \
            '_datetime_' + dt_str + \
            '_shots_' + str(args.shots) + '_ways_' + str(args.classes)

    #print(fileStart)
    if args.only_evaluation is not True:
        if not os.path.exists('results_auto/domain_adaptation/mo_maml_supervised/' + fileStart):
            os.makedirs('results_auto/domain_adaptation/mo_maml_supervised/' + fileStart)
    ...
    if args.only_evaluation is True:
        ...
        print("Model No:",getClassNames(args.checkpoint))
        ...
        train(maml_model, train_set, None , test_set, 'results_auto/domain_adaptation/mo_maml_supervised/' + fileStart, **train_kwargs(args))
        print("Training Complete\n\n")
        
        save_path = 'results_auto/domain_adaptation/mo_maml_supervised/' + fileStart + '/' + 'final_model.pt'
    ...
    UU_accuracies, UU_accuracy, UU_variation = evaluate(maml_model, test_set, **eval_kwargs)
    print('Accuracy: ' + str(UU_accuracy) + '; Variation: ' + str(UU_variation))

    print("\nEvaluating for Unseen Domain and Seen Classes:")
    US_accuracies, US_accuracy, US_variation = evaluate(maml_model, train_set, **eval_kwargs)
    ...
    if args.only_evaluation is not True:
        if not os.path.exists('results_auto/domain_adaptation/mo_maml_supervised/' + fileStart):
            os.makedirs('results_auto/domain_adaptation/mo_maml_supervised/' + fileStart)
        save_path = 'results_auto/domain_adaptation/mo_maml_supervised/' + fileStart + '/' + 'results.npz'
